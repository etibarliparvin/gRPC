...
public class TransferStreamingRequest implements StreamObserver<TransferRequest> {
    ...
    public void onNext(TransferRequest request) {
        ...
        Account fromAccountInfo = Account.newBuilder()
                .setAccountNumber(fromAccount).setAmount(AccountDatabase.getBalance(fromAccount)).build();
        Account toAccountInfo = Account.newBuilder()
                .setAccountNumber(toAccount).setAmount(AccountDatabase.getBalance(toAccount)).build();
